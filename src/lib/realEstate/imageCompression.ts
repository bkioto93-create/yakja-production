// مسیر فایل: src/lib/realEstate/imageCompression.ts
// تسک ۵ فاز ۰۵ — فشرده‌سازی تصویر سمت کلاینت پیش از فرستادن به Storage.
// دقیقاً هم‌الگو با src/lib/marketplace/imageCompression.ts (فاز ۰۲، تسک ۵)، طبق متن دقیق تسک
// («مطابق استاندارد فاز ۰۲»). هدف: هر تصویر نهایی حدود ۱۵۰ تا ۲۵۰ کیلوبایت باشد (بند ۵.۳ و بند ۱۱
// سند راهبردی — بهینه‌سازی برای اینترنت ۲G/۳G و گوشی‌های ارزان‌قیمت رایج در افغانستان). عدد ۱۵۰
// کیلوبایت به‌عنوان کف سخت‌گیرانه اعمال نشده (تصویر ساده‌تر طبیعتاً کوچک‌تر هم می‌شود که خودش
// مطلوب‌تر است)؛ فقط سقف ۲۵۰ کیلوبایت به‌طور فعال با کاهش پلکانی کیفیت کنترل می‌شود.
