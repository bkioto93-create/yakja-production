// مسیر فایل: src/app/[lang]/listings/new/page.tsx
...
export default async function NewListingPage({
    ...
    return (
      <div className="flex flex-col min-h-[70vh] items-center justify-center px-6 py-10">
        <Card className="p-6 flex flex-col items-center text-center gap-3 max-w-sm w-full">
          ...
          <h2 className="font-extrabold text-text-main">{dict.marketplace.wizard.loginRequiredTitle}</h2>
          <p className="text-sm text-text-muted">{dict.marketplace.wizard.loginRequiredDesc}</p>
          <Link href={`/${lang}/auth/login`} className="w-full">
            <Button variant="primary" fullWidth>
              {dict.marketplace.wizard.loginRequiredButton}
            </Button>
          </Link>
        </Card>
      </div>
  ...
  return (
    <div className="flex flex-col px-4 md:px-0 py-6 max-w-lg md:max-w-xl mx-auto w-full gap-4">
      <h1 className="text-xl font-extrabold text-text-main">{dict.marketplace.wizard.title}</h1>
      <NewListingWizard
        lang={lang as Locale}
        dict={dict}
