...
//
// 🛠️ به‌روزرسانی (بازخورد تازه‌ی کارفرما — جای بخش VIP): «این بخش [VIP] نباید بالای بالای
// صفحه باشد». VipHomeBanner و VipPitchSection که قبلاً بلافاصله بعد از «دسترسی سریع» بودند،
// جابه‌جا شدند: VipHomeBanner حالا بین دو دسته‌بندیِ اول (حمل‌ونقل/خدمات) و دوتای بعدی
// (کالا/املاک) است — یعنی وسطِ فیدِ محتوای واقعی، مثل یک بنرِ میان‌متنی در اپ‌های خرید استاندارد؛
// VipPitchSection حالا بعد از هر ۴ دسته‌بندی/ردیفِ محتوا، درست پیش از «چرا یکجا؟» است. جزئیات
// دقیقِ جای هرکدام را در همان JSX پایین همین فایل، کنار خودشان، ببینید.
    ...
    <div className="flex flex-col min-h-screen bg-bg-base sm:border-x md:border-x-0 border-slate-100 max-w-md md:max-w-none mx-auto shadow-sm md:shadow-none pb-8 relative animate-fade-in w-full overflow-hidden">
      ...
      <div className="flex-1 px-[24px] md:px-12 mt-8 w-full z-10 flex flex-col justify-start align-top content-start justify-items-start space-y-12">
         ...
         <div className="-mx-[24px] md:mx-0 space-y-8">
           ...
           {/* بنر VIP — 🛠️ جابه‌جایی (بازخورد کارفرما: «این بخش نباید بالای بالای صفحه باشد»؛
               قبلاً بلافاصله بعد از «دسترسی سریع» بود). جای تازه: دقیقاً بعد از دو دسته‌بندیِ
               اول (حمل‌ونقل، خدمات) و پیش از دوتای بعدی — یعنی وسطِ فیدِ محتوای واقعی، مثل یک
               بنرِ تبلیغاتیِ میان‌متنی در اپ‌های خرید استاندارد، نه اولین چیزی که کاربر می‌بیند. */}
           <div className="px-[24px] md:px-0">
         ...
         {/* بخشِ ترغیبیِ «چرا VIP نتیجه‌ی بهتری می‌آورد؟» — 🛠️ جابه‌جایی (همان بازخورد بالا):
             جای تازه، بعد از هر ۴ دسته‌بندی/ردیفِ محتوا، پیش از «چرا یکجا؟» — یعنی آخرین پیامِ
             ترغیبی پیش از بخش‌های عمومی/بازاریابیِ پایین صفحه، بعد از این‌که کاربر کل محتوای
             واقعی را دیده. کامپوننتِ مشترک، رجوع کنید به src/components/home/VipPitchSection.tsx
             (همان بخش، عیناً، در خودِ صفحه‌ی VIP هم هست). */}
         <VipPitchSection
           dict={dict.vip.pitch}
           ctaHref={`/${lang}/vip`}
